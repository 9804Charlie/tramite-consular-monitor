...
def run_once(force: bool = False) -> int:
    ...
    if not is_status_page(html):
        ...
        motivo = extract_alert(html) or "captcha incorrecto o datos no reconocidos"
        log(f"Respuesta != pagina de estado ({motivo}). Snapshot: {snap.name}")
        tg.send_message(f"❌ No entro: {motivo}. Se reintenta en la proxima "
                        f"pasada del cron (~15 min) o con /revisar.")
        save_state(s, state)
        return 0
    ...
    if prev_digest is None:
        ...
        tg.send_message(f"🫀 cron OK, sin cambios ({_ts(now_ts)})")
